	...
	return (
		...
		>
			<div className="flex flex-col gap-4">
				<header>
					<h2 className="text-lg font-semibold" title={title}>{title}</h2>
					<p className="text-xs text-gray-400 mt-1">By {authors?.length ? authors.join(', ') : 'Unknown author'}</p>
			...
			<div className="space-y-4 pt-2">
				...
				<footer className="text-[10px] text-gray-500 pt-2 border-t border-gray-700 mt-2">
					Data sourced from{' '}
					<Link href="https://arxiv.org" target="_blank" className="underline" aria-label="arXiv.org">
						arXiv.org
					</Link>
